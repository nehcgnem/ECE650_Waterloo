...
void parseEdge(string s, vector<int> & v)
{
	...
	while(std::getline(ss, temp, ','))
	{
     ...
     cerr <<"Error:" <<e.what(); // information from length_error printed
 }
 return;
}
...
{
	int it = *max_element(begin(v), end(v)); // c++11
	if (it >= n ){  
...
void cleargraph(vector<int> Graph[], int n)
{
...
{ 
	...
	{ 
		cerr << "Error: source and destination"
