...
def intersect(a, b, c, d):
    de1 = ((b[1] - a[1]) * (c[0] - d[0]) - (b[0] - a[0]) * (c[1] - d[1]))
    de2 = ((b[0] - a[0]) * (c[1] - d[1]) - (b[1] - a[1]) * (c[0] - d[0]))
    if de1 != 0 or de2 != 0:
        x = ((b[0] - a[0]) * (c[0] - d[0]) * (c[1] - a[1]) - (c[0]) * (b[0] - a[0]) * (c[1] - d[1]) + (a[0]) * (b[1] - a[1]) * (c[0] - d[0])) / de1
        y = ((b[1] - a[1]) * (c[1] - d[1]) * (c[0] - a[0]) - (c[1]) * (b[1] - a[1]) * (c[0] - d[0]) + (a[1]) * (b[0] - a[0]) * (c[1] - d[1])) / de2
        if (x - (a[0])) * (x - (b[0])) <= 0 and (x - (c[0])) * (x - (d[0])) <= 0 and (y - (a[1])) * (y - (b[1])) <= 0 and (y - (c[1])) * (y - (d[1])) <= 0:
            return (x, y)
        else:
            return None 
...
class Edge(object):
    ...
    def print_vertice(self):
        ...
        # print("}")
        try:
            sys.stdout.write('V ' + str(sorted(self.vertice.keys())[-1]+1) + '\n')
        ...
        # print("V {}".format(sorted(self.vertice.keys())[-1]))
...
class Graph(object):
    ...
    def remove_street(self,streetname):
        if streetname.lower() in self.state:
            ...
            #     edge.reset()
            # self.history = {} # clear history when all street removed
            # self.vertices = {} # clear everything 
            #print(self.state) # debug 
        elif streetname:
            print('Error: street \"{0}\" does not exist'.format(streetname))
        else:
        ...
        while True:
            ...
            if format_check(line):
                ...
                if option[0].replace(" ", "") == 'a':  
                    ...
                    if (graph.check_empty()):
                        edge.reset()
                        graph.reset() # fix for a3
